...
public class RenderWings extends ModelBase {
    ...
    public RenderWings() {
        if(WingMode.equals("Dragon")) { this.location = new ResourceLocation("wawa/wings/DragonWings.png"); } else if(WingMode.equals("Simple")) { this.location = new ResourceLocation("fdpclient/wings/NeonWings.png"); }
    ...
    public void renderWings(float partialTicks) {
        ...
        double rotate = this.interpolate( mc.player.prevRenderYawOffset, mc.player.renderYawOffset, partialTicks);
        GL11.glPushMatrix();
        ...
        if(Wings.getColourType().get().equals("Chroma")){ RenderUtils.glColor(ColorUtils.rainbow(), 255);} else if(Wings.getColourType().get().equals("Custom")){RenderUtils.glColor(new Color(Wings.getCR().get(), Wings.getCG().get(), Wings.getCB().get()), 255);} else { GL11.glColor3f(1,1,1);}
