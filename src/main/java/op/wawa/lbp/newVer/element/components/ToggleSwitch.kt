package op.wawa.lbp.newVer.element.components

import op.wawa.lbp.newVer.extensions.animLinear
...
class ToggleSwitch {
    ...
    fun onDraw(x: Float, y: Float, width: Float, height: Float, bgColor: Color, accentColor: Color) {
        ...
        RenderUtils.drawRoundedRect(x - 0.5F, y - 0.5F, x + width + 0.5F, y + height + 0.5F,
            ((height + 1F) / 2F).toInt(), borderColor.rgb)
        RenderUtils.drawRoundedRect(x, y, x + width, y + height, (height / 2F).toInt(), mainColor.rgb)
