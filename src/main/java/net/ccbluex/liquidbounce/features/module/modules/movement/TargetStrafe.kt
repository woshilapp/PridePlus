...
class TargetStrafe : Module() {
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        if (renderModeValue.get() != "None" && canStrafe(target)) {
            ...
            if (renderModeValue.get().equals("Circle", ignoreCase = true)) {
                ...
                for (i in 0..359) {
                    val rainbow = Color(
                        Color.HSBtoRGB(
                            ((mc.player.ticksExisted / 70.0 + sin(i / 50.0 * 1.75)) % 1.0f).toFloat(),
                            0.7f,
                            1.0f
                        )
                    )
        ...
        fun onMove(event: MoveEvent) {
            if(doStrafe && (!onGroundValue.get() || mc.player.onGround)) {
    ...
    private fun isVoid(xPos: Int, zPos: Int): Boolean {
        if (mc.player.posY < 0.0) return true
        var off = 0
        while (off < mc.player.posY.toInt() + 2) {
            val bb = mc.player.entityBoundingBox.offset(xPos.toDouble(), -off.toDouble(), zPos.toDouble())
