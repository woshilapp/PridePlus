...
class KeyStrokes : Element(5.0, 25.0, 1.25F, Side.default()) {
    ...
    override fun drawElement(): Border? {
        ...
        for (keyStroke in keys) {
            keyStroke.render(animSpeedValue.get(), backGroundColor, textColor, highLightPercent.get(), outline.get(), outlineBoldValue.get(), fontValue.get(), this.renderX.toFloat(), this.renderY.toFloat(), scale)
        }
...
class KeyStroke(val key: KeyBinding, val posX: Int, val posY: Int, val width: Int, val height: Int) {
    ...
    fun render(
        ...
        RenderUtils.drawRoundedRect(0F, 0F, width.toFloat(), height.toFloat(),3, rectColor.rgb)
        ...
        //傻逼更新速度 大神在飘hyp VIV
        //3542578348是吧 不是 是软晴
        //jetbrains idea 就是歌姬吧
        //傻逼更新速度操死你妈了 ?ssshsbbsdbasdjsadsdjsjdajsdjsadjsadjadasdada
