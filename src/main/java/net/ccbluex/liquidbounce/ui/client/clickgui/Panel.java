...
public abstract class Panel extends MinecraftInstance {
    ...
    public void drawScreen(int mouseX, int mouseY, float button) {
        ...
        final int maxElements = ((ClickGUI) Objects.requireNonNull(Pride.moduleManager.getModule(ClickGUI.class))).maxElementsValue.get();
        ...
        Pride.clickGui.style.drawPanel(mouseX, mouseY, this);
    ...
    private int getElementsHeight() {
        ...
        int count = 0;
        for(final Element element : elements) {
            if (count >= ((ClickGUI) Objects.requireNonNull(Pride.moduleManager.getModule(ClickGUI.class))).maxElementsValue.get())
                continue;
    ...
    boolean isHovering(int mouseX, int mouseY) {
        final float textWidth = mc.fontRenderer.getStringWidth(StringUtils.stripControlCodes(name)) - 100F;
        return mouseX >= x - textWidth / 2F - 19F && mouseX <= x - textWidth / 2F + mc.fontRenderer.getStringWidth(StringUtils.stripControlCodes(name)) + 19F && mouseY >= y && mouseY <= y + height - (open ? 2 : 0);
    }
}
