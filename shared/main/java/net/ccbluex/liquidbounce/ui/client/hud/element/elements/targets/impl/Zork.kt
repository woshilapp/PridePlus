...
import net.ccbluex.liquidbounce.utils.render.RenderUtils.drawScaledCustomSizeModalCircle
...
class Zork(inst: Target)  : TargetStyle("Zork",inst,true){
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
        ...
        width = 26f + Fonts.font30.getStringWidth(entity.name.toString()).coerceAtLeast(90)
        val size = 24f
        ...
        RoundedUtil.drawRound(
            34F,
            28F,
            (width - 42) * (mc.thePlayer!!.totalArmorValue / 20F).coerceIn(0F, 1F),
            2F,
            0.8F,
            Color(255, 255, 255)
            )
