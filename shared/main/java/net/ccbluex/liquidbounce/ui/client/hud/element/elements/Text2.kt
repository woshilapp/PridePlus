...
class Text2(
    ...
    override fun drawElement(): Border? {
        ...
        val fontRenderer = FontLoaders.F24
        ...
        val expand = FontLoaders.F24.height * rectExpandValue.get()
        when (rectValue.get().toLowerCase()) {
            "normal" -> {
                RenderUtils.drawRect(-expand, -expand, FontLoaders.F24.getStringWidth(displayText) + expand, FontLoaders.F24.height + expand, rectColor)
            }
            "rnormal" -> {
                RenderUtils.drawRect(-expand, -expand - 1, FontLoaders.F24.getStringWidth(displayText) + expand, -expand, ColorUtils.rainbow())
                RenderUtils.drawRect(-expand, -expand, FontLoaders.F24.getStringWidth(displayText) + expand, FontLoaders.F24.height + expand, rectColor)
            }
            "onetap" -> {
                RenderUtils.drawRect(-4.0f, -8.0f, (FontLoaders.F24.getStringWidth(displayText) + 3).toFloat(), FontLoaders.F24.height.toFloat(), Color(43, 43, 43).rgb)
                RenderUtils.drawGradientSidewaysH(-3.0, -7.0, FontLoaders.F24.getStringWidth(displayText) + 2.0, -3.0, Color(rectColor).darker().rgb, rectColor)
            }
            "skeet" -> {
                RenderUtils.drawRect(-11, -11, (FontLoaders.F24.getStringWidth(displayText) + 10), FontLoaders.F24.height + 8, Color(0, 0, 0).rgb)
                RenderUtils.drawOutlinedRect(-10, -10, (FontLoaders.F24.getStringWidth(displayText) + 9), FontLoaders.F24.height + 7, 8, Color(59, 59, 59), Color(59, 59, 59))
                RenderUtils.drawOutlinedRect(-9, -9, (FontLoaders.F24.getStringWidth(displayText) + 8), FontLoaders.F24.height + 6, 4, Color(59, 59, 59), Color(40, 40, 40))
                RenderUtils.drawOutlinedRect(-4, -4, (FontLoaders.F24.getStringWidth(displayText) + 3), FontLoaders.F24.height + 1, 1, Color(18, 18, 18), Color(0, 0, 0))
            }
            "logo" -> {
        ...
        if(!rectValue.get().contains("Logo")) {
            ...
            FontLoaders.F24.drawString(
                "By WaWa", 5.0f, 37.0f,Color(255,255,255,220).rgb
            )
        }
