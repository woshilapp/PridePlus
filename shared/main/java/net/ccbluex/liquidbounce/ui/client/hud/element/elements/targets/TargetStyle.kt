...
abstract class TargetStyle(val name: String, val targetInstance: Target, val shaderSupport: Boolean): MinecraftInstance() {
    ...
    var easingHealth = 0F
    var easingArmor = 0F
    val shieldIcon = classProvider.createResourceLocation("liquidbounce/shield.png")
    ...
    val shadowOpaque: Color
        ...
        }, (1F - targetInstance.animProgress).toInt()
        )
    ...
    open fun updateAnimArmor(targetArmor: Float) {
        if (targetInstance.noAnimValue.get())
            easingArmor = targetArmor

        else
            easingArmor += ((targetArmor - easingArmor) / 2.0F.pow(10.0F - targetInstance.globalAnimSpeed.get())) * RenderUtils.deltaTime
    }

    open fun handleDamage(player: IEntityLivingBase) {}

    open fun handleBlur(player: IEntityLivingBase) {}
    
    open fun handleShadowCut(player: IEntityLivingBase) {}
    open fun handleShadow(player: IEntityLivingBase) {}
    ...
    fun getColor(color: Color) = ColorUtils.reAlpha(color,
        (color.alpha / 255F * (1F - targetInstance.getFadeProgress())).toInt()
    )
