...
class ScoreboardElement(x: Double = 5.0, y: Double = 0.0, scale: Float = 1F,
    ...
    private val rectshadowValue = BoolValue("RectShadow", true)
    ...
    private val fontValue = FontLoaders.F14
    ...
    override fun drawElement(): Border? {
        ...
        val maxHeight = scoreCollection.size * fontRenderer.height
        val l1 = -maxWidth - 3 - if (rectValue.get()) 3 else 0



        RenderUtils.drawRect(l1 - 2, -2, 5, (maxHeight + fontRenderer.height), backColor)
        if(rectshadowValue.get()){
            RenderUtils.drawShadowWithCustomAlpha(l1 - 2f, -2f, -l1+7f, maxHeight + fontRenderer.height + 2f, 255f)
        }
        scoreCollection.forEachIndexed { index, score ->
            ...
            val height = maxHeight - index * fontRenderer.height.toFloat()
            ...
            if (index == scoreCollection.size - 1) {
                ...
                fontRenderer.drawString(displayName, (l1 + maxWidth / 2 - fontRenderer.getStringWidth(displayName) / 2).toFloat(), (height -
                        fontRenderer.height), textColor, shadowValue.get())
            }
            ...
            if (rectValue.get()) {
                ...
                RenderUtils.drawRect(2F, if (index == scoreCollection.size - 1) -2F else height, 5F, if (index == 0) fontRenderer.height.toFloat() else height + fontRenderer.height * 2F, rectColor)
            }
        }

        return Border(-maxWidth.toFloat() - 5 - if (rectValue.get()) 3 else 0, -2F, 5F, maxHeight + fontRenderer.height.toFloat())
    }
