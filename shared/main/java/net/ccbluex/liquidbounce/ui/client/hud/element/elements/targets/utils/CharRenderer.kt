...
class CharRenderer(val small: Boolean): MinecraftInstance() {
    ...
    fun renderChar(number: Float, orgX: Float, orgY: Float, initX: Float, initY: Float, scaleX: Float, scaleY: Float, shadow: Boolean, fontSpeed: Float, color: Int): Float {
        ...
        val fontRend = if (small) Fonts.font40 else Fonts.fontBold72
        ...
        for (char in reFormat.toCharArray()) {
            moveX[indexX] = AnimationUtil.animate(animX, moveX[indexX], fontSpeed * 0.025F * delta)
            animX = moveX[indexX]
            ...
            if (pos >= 0) {
                moveY[indexY] = AnimationUtil.animate(expectAnim, moveY[indexY], fontSpeed * 0.02F * delta)
