...
import me.utils.render.ScaleUtils
...
class Target2 : Element(-46.0,-40.0,1F,Side(Side.Horizontal.MIDDLE,Side.Vertical.MIDDLE)) {
    ...
    val blurStrength = FloatValue("BlurStrength",10F,0F,20F)
    ...
    private val riseParticleList = mutableListOf<Target2.RiseParticle>()
    ...
    private fun rise2(target: IEntityLivingBase, easingHealth: Float) {
        ...
        RoundedUtil.drawGradientRound(40F, 25F, 90F * (easingHealth / 20f), 5F, 3F, Color(70, 200, 200), Color.WHITE, Color(70, 200, 200), Color.WHITE)
        Fonts.font35.drawStringWithShadow("Name: ", 40, 8, Color.WHITE.rgb)
        Fonts.font35.drawStringWithShadow(target.name!!, 40 + Fonts.font35.getStringWidth("Name: "), 8, Color(70, 150, 140).rgb)
        Fonts.font35.drawStringWithShadow(decimalFormat.format(easingHealth), 135, 24, Color(70, 150, 140).rgb)
        ...
        if (playerInfo != null) {
            ...
            if (target.hurtTime >= 9) {
                for (i in 0 until CountValue.get()) {
                    riseParticleList.add(Target2.RiseParticle())
                }
            }
    ...
    private fun drawLw(target: IEntityLivingBase, easingHealth: Float) {
        ...
        if (blur.get()) {
            GL11.glTranslated(-renderX, -renderY, 0.0)
            GL11.glPushMatrix()
            BlurUtils.blurArea(renderX.toFloat() + 1f, renderY.toFloat(), 148.95f,50.0f,blurStrength.get())
            GL11.glPopMatrix()
            GL11.glTranslated(renderX, renderY, 0.0)
        }
        RenderUtils.drawShadow(1, 0, 148, 50)
        // draw entity
        // target text
        FontLoaders.F22.drawStringWithShadow("${target.name}", 45f, 10f, Color.WHITE.rgb)
...
//        )
    ...
    private fun drawNovo2(target: IEntityLivingBase, easingHealth: Float) {
        ...
        if(novoshadow.get()){
            ...
            ShadowUtils.shadow(shadowValue.get(),{
                ...
                GL11.glTranslated(renderX, renderY, 0.0)
                GL11.glScalef(scale, scale, scale)
                RenderUtils.drawRoundedRect2(0F, 0F, 94f + FontLoaders.SB18.getStringWidth(target.name) + (easingHealth / 20f), 45F, novoradi.get(), Color(0, 0, 0).rgb)
                GL11.glPopMatrix()
        ...
        RenderUtils.drawRoundedRect2(0F, 0F, 94f + FontLoaders.SB18.getStringWidth(target.name) + (easingHealth / 20f), 45F, novoradi.get(), Color(0, 0, 0, alpha.get()).rgb)
        RoundedUtil.drawGradientRound(40F, 27F, 40F * (easingHealth / 20f) + FontLoaders.SB18.getStringWidth(target.name)-7f, 5F, 3F, Color(CustomUI.r.get(),CustomUI.g.get(),CustomUI.b.get()), Color.WHITE, Color(CustomUI.r2.get(),CustomUI.g2.get(),CustomUI.b2.get()), Color.WHITE)
        if (blur.get()) {

            GL11.glTranslated(-renderX, -renderY, 0.0)
            GL11.glPushMatrix()
            BlurUtils.CustomBlurRoundArea(renderX.toFloat(), renderY.toFloat(), 94f + FontLoaders.SB18.getStringWidth(target.name) + (easingHealth / 20f),45f, novoradi.get(), blurstr.get())
            GL11.glPopMatrix()
            GL11.glTranslated(renderX, renderY, 0.0)
        }
        RoundedUtil.drawGradientRound(40F, 27F, 40F * (easingHealth / 20f) + FontLoaders.SB18.getStringWidth(target.name)-7f, 5F, 3F, Color(CustomUI.r.get(),CustomUI.g.get(),CustomUI.b.get()), Color.WHITE, Color(CustomUI.r2.get(),CustomUI.g2.get(),CustomUI.b2.get()), Color.WHITE)
        Fonts.font35.drawStringWithShadow("Name: ", 40, 10, Color.WHITE.rgb)
        FontLoaders.SB18.drawStringWithShadow(target.name!!,
            (42 + Fonts.font35.getStringWidth("Name: ")).toDouble(), 9.0, Color(CustomUI.r.get(),CustomUI.g.get(),CustomUI.b.get()).rgb)
        Fonts.font35.drawStringWithShadow(decimalFormat.format(easingHealth), ((94f + FontLoaders.SB18.getStringWidth(target.name) + (easingHealth / 20f) - 18f).toInt()), 26, Color(CustomUI.r.get(),CustomUI.g.get(),CustomUI.b.get()).rgb)
    ...
    private fun drawNovo(target: IEntityLivingBase, easingHealth: Float) {
        ...
        FontLoaders.F22.drawStringWithShadow("Health: ${Math.round(easingHealth)}", 47.0,
            FontLoaders.F22.FONT_HEIGHT.toDouble() + 3f, Color.WHITE.rgb)
        FontLoaders.F22.drawStringWithShadow("Distance: ${Math.round(target.getDistance(mc.thePlayer!!.posX,mc.thePlayer!!.posY,mc.thePlayer!!.posZ))}m",
            47.0, (FontLoaders.F22.FONT_HEIGHT + 13f).toDouble(), Color.WHITE.rgb)
...
//        )
    ...
    private fun drawRound(target: IEntityLivingBase,easingHealth: Float){
        ...
        if (CustomUI.Chinese.get()){
            FontLoaders.F22.drawStringWithShadow("目标名称: ${target.name}", 45f, 7f, Color.WHITE.rgb)
            FontLoaders.F22.drawStringWithShadow("血量: ${Math.round(easingHealth)}",45f,4f + FontLoaders.SB20.height - 5f,Color.WHITE.rgb)
            FontLoaders.F22.drawStringWithShadow("距离: ${Math.round(target.getDistance(mc.thePlayer!!.posX,mc.thePlayer!!.posY,mc.thePlayer!!.posZ))}米",45f,FontLoaders.SB20.height * 2 - 10f,Color.WHITE.rgb)
        }
        else {
            FontLoaders.SB20.drawStringWithShadow("${target.name}", 45f, 7f, Color(CustomUI.r2.get(),CustomUI.g2.get(),CustomUI.b2.get()).rgb)
            Fonts.font40.drawStringWithShadow("Health: ${Math.round(easingHealth)}",44,4 + FontLoaders.SB20.height - 5,Color.WHITE.rgb)
            Fonts.font40.drawStringWithShadow("Distance: ${Math.round(target.getDistance(mc.thePlayer!!.posX,mc.thePlayer!!.posY,mc.thePlayer!!.posZ))}m",44, FontLoaders.SB20.height * 2 - 10,Color.WHITE.rgb)
        }
    ...
    private fun drawZork(target: IEntityLivingBase, easingHealth: Float) {
        ...
        // draw entity
        // target text
        FontLoaders.F22.drawStringWithShadow("${target.name}", 45f, 6f, Color.WHITE.rgb)
        val df = DecimalFormat("0.00"); //bar
...
//    }
    private fun liquidwing2(target: IEntityLivingBase,easingHealth: Float) {
        ...
        ShadowUtils.shadow(10F, {
            ...
            GL11.glTranslated(renderX, renderY, 0.0)
            GL11.glScalef(scale, scale, scale)
            RenderUtils.drawRoundedRect(0F, 0F, 150F, 50F, 5, Color(0, 0, 0, 255).rgb)
            GL11.glPopMatrix()
        }, {
            GL11.glPushMatrix()
            GL11.glTranslated(renderX, renderY, 0.0)
        ...
        GL11.glTranslated(renderX, renderY, 0.0)
        //draw Background
        RenderUtils.drawRoundedRect(0F,0F,150F,50F,5,Color(255 ,247 ,230,170).rgb)
        ...
        Fonts.font35.drawString(target.name!!, 46, 13, Color(255 ,247 ,230,255).rgb)
        ...
        Fonts.font30.drawString(health2.toString(), 48, 31, Color(0 ,0 ,0,255).rgb)
    ...
    private fun liquidwing(target: IEntityLivingBase, easingHealth: Float) {
        ...
        if (blur.get()) {
            GL11.glTranslated(-renderX, -renderY, 0.0)
            GL11.glPushMatrix()
            BlurUtils.CustomBlurRoundArea(renderX.toFloat(), renderY.toFloat(), 150F, 40F, 4f,blurStrength.get())
            GL11.glPopMatrix()
            GL11.glTranslated(renderX, renderY, 0.0)
        }
        if (bg.get()) {
        ...
        "${decimalFormat.format((easingHealth / target.maxHealth) * 100)}%".also {
            Fonts.font40.drawString(
                it,
                (40f + (easingHealth / target.maxHealth) * 105f - Fonts.font40.getStringWidth(it)).coerceAtLeast(
                    40f
                ), 19F, -1, false
            )
        }
        ...
        Fonts.font40.drawString(target.name!!, 36, 8, -1)
    ...
    private fun distance(target: IEntityLivingBase, easingHealth: Float) {
        ...
        Fonts.font35.drawString("Distance:   " + ChatFormatting.WHITE + Math.round(target.getDistance(mc.thePlayer!!.posX,mc.thePlayer!!.posY,mc.thePlayer!!.posZ)) + "m", 36, 18, Color(41,132,163).rgb)
        ...
        Fonts.font35.drawCenteredString(Math.round(easingHealth).toString(), 123.1f, 12f, -1)// 你还没口血呢，当然是完整的圆什么
    ...
    private fun rise111(target: IEntityLivingBase, easingHealth: Float) {
        ...
        Fonts.font40.drawString(target.name!!, 40, 8, -1)
    ...
    private fun getTBorder():Border?{
        return when(modeValue.get().toLowerCase()){
            "zork" -> Border(0F,0F,150F,50F)
            "newliquidwing"-> Border(0F,0F,150F,50F)
            "newliquidwing2"-> Border(0F,0F,150F,50F)
            "liquidwing"-> Border(0F,0F,150F,50F)
            "novoline"-> Border(0F,0F,150F,60F)
            "novo"-> Border(0F,0F,150F,60F)
            "rise"->Border(0F,0F,150F,60F)
            "distance"->Border(0F,0F,150F,30F)
            "rise2" -> Border(0F, 0F, 150F, 40F)
            "newround" -> Border(0F, 0F, 150F, 40F)
            else -> null
        }
