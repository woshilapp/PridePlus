...
//import skid.flux.fontRenderer.RapeMasterFontManager
//import studio.mcmodule.czf233.utils.TenacityRectUtil
...
class Target2 : Element(0.0,0.0,1F,Side(Side.Horizontal.MIDDLE,Side.Vertical.MIDDLE)) {
    ...
    fun getColor(color: Color) = ColorUtils.reAlpha(color, (color.alpha / 255F * (1F - getFadeProgress())).toInt())
    ...
    override fun drawElement(): Border? {
        ...
        barColor = ColorUtils.reAlpha(preBarColor, (preBarColor.alpha / 255F * (1F - animProgress)).toInt())
        bgColor = ColorUtils.reAlpha(preBgColor, (preBgColor.alpha / 255F * (1F - animProgress)).toInt())
    ...
    private fun drawRice(entity: IEntityLivingBase,easingHealth: Float) {
            ...
            if (gotDamaged) {
                for (j in 0..(generateAmountValue.get())) {
                    ...
                    val color2 = RenderUtils.getGradientOffset(
                        ...
                        Color(gredValue.get(), ggreenValue.get(), gblueValue.get()),
                        10.0
                    )
                    particleList.add(
                        Particle(
        ...
        // custom head
        val scaleHT = (entity.hurtTime.toFloat() / Math.random().toFloat()).coerceIn(0F, 1F)
...
//        GL11.glTranslated(renderX, renderY, 0.0)
    ...
    private fun drawZamorozka(target: IEntityLivingBase, easingHealth: Float){
        ...
        font.drawCenteredString("${target.name} ${if(playerInfo!!.responseTime.coerceAtLeast(0)!=-1) { "§f${playerInfo.responseTime.coerceAtLeast(0)}ms" } else { "" }}", 93f, 16f-font.fontHeight, ColorUtils.rainbow().rgb, false)
