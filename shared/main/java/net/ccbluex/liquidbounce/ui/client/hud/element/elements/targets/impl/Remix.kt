...
class Remix(inst: Target): TargetStyle("Remix", inst, false) {
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
        ...
        RenderUtils.newDrawRect(101F, 17F, 117F, 33F, getColor(Color(95, 95, 95)).rgb)
        // name
        Fonts.font30.drawStringWithShadow(entity.name!!, 41, 5, getColor(-1).rgb)
        ...
        if (mc.netHandler.getPlayerInfo(entity.uniqueID) != null) {
            ...
            GL11.glPushMatrix()
            GL11.glTranslatef(142F - Fonts.font30.getStringWidth(stringTime) / 2F, 28F, 0F)
            GL11.glScalef(0.5F, 0.5F, 0.5F)
            Fonts.font30.drawStringWithShadow(stringTime, 0, 0, getColor(-1).rgb)
            GL11.glPopMatrix()
        }
